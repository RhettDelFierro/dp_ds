...
func (handler *SQLHandler) sendQuery(q string) ([]Animal, error) {
	...
	for rows.Next() {
		a := Animal{}
		err := rows.Scan(&a.ID, &a.AnimalType, &a.Zone, &a.Age)
		if err != nil {
			log.Println(err)
			continue
		}
...
func (handler *SQLHandler) GetDynoByNickname(nickname string) (Animal, error) {
	...
	err := row.Scan(&a.ID, &a.AnimalType, &a.Zone, &a.Age)
	return a, err
}
...
func (handler *SQLHandler) AddAnimal(a Animal) error {
	_, err := handler.Exec(
		fmt.Sprintf("Insert into Animals (Animal_type, nickname, zone, age) values ('%s', '%s', '%d' ,'%d')",
